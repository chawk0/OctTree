...
void World::_buildRenderBuffer()
{
	...
	try
	{
		// build each segment's render buffer
		// set to 2 for now: terrain segment and water segment
		int segmentCount = 2;
		Segment segment;
		// holds which voxels of the current segment have visible faces
		vector<FaceData> _visibleVoxels;
		int segmentVoxelID, n;
		bool checkWater;
		...
		for (int s = 0; s < segmentCount; ++s)
		{
			...
			// hack for now to make the first segment (terrain) visible through water
			if (s == 0)
			{
				checkWater = true;
				segmentVoxelID = 1;
				segment.name = "terrain";
			}
			else
			{
				checkWater = false;
			...
			for (int x = 0; x < _width; ++x)
			{
				for (int y = 0; y < _height; ++y)
				{
					for (int z = 0; z < _depth; ++z)
					{
						...
						// check only voxels that match the current segment's voxel type
						if (_grid[index] == segmentVoxelID)
						{
							...
							n = _computeVisibleFaces(x, y, z, &fd, checkWater);
			...
			int vertDataSize = 5;
			segment.buffer.data = new float[segment.faceCount * 2 * 3 * vertDataSize];
			segment.buffer.len = segment.faceCount * 2 * 3 * vertDataSize * sizeof(float);
			segment.buffer.offset = segment.faceCount * 2 * 3 * 3 * sizeof(float);
			segment.buffer.elementCount = segment.faceCount * 2 * 3;
			float* ptr = segment.buffer.data;
			float* ptr2 = segment.buffer.data + segment.faceCount * 2 * 3 * 3;
			...
			glBufferData(GL_ARRAY_BUFFER, segment.buffer.len, segment.buffer.data, GL_STATIC_DRAW);
			glEnableVertexAttribArray(0);
			// stream 0 is position data, 3 floats
			glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
			glEnableVertexAttribArray(3);
			// stream 3 is uv data, 2 floats
			glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, 0, (GLvoid*)segment.buffer.offset);
...
void World::render(Camera& camera, Shader& program)
{
